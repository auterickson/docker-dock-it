...
$query = "SELECT characterId, name, description
    FROM Characters
...
$characterCount = mysqli_num_rows($result);
echo "<p>Found $characterCount characters.</p>";
...
<div class="box">
    ...
    <table class="table table-striped">
        ...
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            ...
            <tr>
                ...
                <td><a href="characterinfo.php?id=<?= $row['characterId'] ?>"><?= $row['name'] ?></a></td>
            </tr>
            <?php
        }
        ?>
    ...
    <nav aria-label="...">
        <ul class="pagination">
            <li class="page-item ">
                <a class="page-link" href="?search=<?= $search ?>&sort=<?= $sortBy ?>&dir=<?= $dir ?>&page=<?= $page - 1 ?>">Previous</a>
            </li>
            <?php
            $pageCount = ceil($characterCount / $perPage);
            $maxLinks = 12;
    ...
    <?php if ($userLoggedIn) {
        echo "<a href='add-character.php?characterId={$id}' class='btn btn-primary'>Add Character</a>";
    } ?>
